...
2. ACR のアクセスエンドポイントを定義するための変数を作成します。  
...
4. リポジトリの `Secrets` に ACR への接続情報を保存します。  
...
5. リポジトリの [Settings] -> [Secrets] -> [Actions] と辿り、`New repository secret` ボタンをクリックし `Secret` を追加します。  
    ...
    1.  `CONTAINER_REGISTRY_USERNAME` を追加し、上でコピーした ACR の `ユーザー名` を保存します。<br>
    <img src="../images/P2-02-06.png" width="700"><br>
    2. 同様に `CONTAINER_REGISTRY_PASSWORD` を追加し、上でコピーした ACR の `password` を設定します。<br>
